...
<div class="maincontainer">
	...
	<div class="mid">
		<ul class="midlist">
			...
			<li><span class="nowbtn">감독/출연</span></li>
		...
		$.ajax({
			...
			success:function(data){
				...
				$.ajax({
					...
					success:function(data){
						dircell.empty();
						console.log("감독 이미지 데이터:", data);
						if (data && data.trim() !== '') {
							dircell.append('<img src="' + data + '" class="actorimg" alt="감독이미지">');
						} else {
							dircell.append('<img src="/actor_image/빈이미지.jpg" class="actorimg" alt="감독이미지">');
						}
						dircell.append('<div>' + dirn + '</div>');
					},
					error: function() {
						dircell.empty();
						dircell.append('<img src="/actor_image/빈이미지.jpg" class="actorimg" alt="감독이미지">');
				...
				$.ajax({
					...
					traditional: true,
					success: function(data) {
						console.log("배우 이미지 데이터:", data);
						castcell.empty();
						if (data && data.length > 0) {
							$.each(data, function(index, actor) {
								let imagePath = actor.image_path || '/actor_image/빈이미지.jpg';
								castcell.append('<div><img src="' + imagePath + '" class="actorimg" alt="배우 이미지"></div>');
								castcell.append('<div>' + actor.actor + '</div>');
							});
						} else {
							// 배우 이미지가 없을 때 기본 이미지 표시
							$.each(allcastname, function(index, actorName) {
								castcell.append('<div><img src="/actor_image/빈이미지.jpg" class="actorimg" alt="배우 이미지"></div>');
								castcell.append('<div>' + actorName + '</div>');
							});
						}
					},
					error: function() {
						console.log("배우 이미지 로드 실패");
						castcell.empty();
						$.each(allcastname, function(index, actorName) {
							castcell.append('<div><img src="/actor_image/빈이미지.jpg" class="actorimg" alt="배우 이미지"></div>');
							castcell.append('<div>' + actorName + '</div>');
